...
for i, rho in enumerate(rho_order):
    ...
    ax[0].plot(group["npts"], group["time_vbb_mean"],
               'o', label=f"VBBinaryLensing, {rho_labels[i]}",
               color=colors[i % len(colors)], alpha=0.6, ms=10)

# microJAX を後にプロット
for i, rho in enumerate(rho_order):
    group = df[df["rho"] == rho]
    ax[0].plot(group["npts"], group["time_mj_mean"],
               '*', label=f"microjax, {rho_labels[i]}",
...
# ax[1].legend(loc="lower right")  # 必要なら表示
