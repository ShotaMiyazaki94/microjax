...
@jit
def mag_mj(w):
    return mag_limb_dark(w, rho, s=s, q=q, u1 =0.5,
...
def chunked_vmap(func, data, chunk_size=1000):
    results = []
...
for rho in rho_list:
    for npts in npts_list:
        ...
        _ = chunked_vmap(mag_mj, w_test).block_until_ready()
        times = []
        for _ in range(10):
            start = time.time()
            _ = chunked_vmap(mag_mj, w_test).block_until_ready()
...
df.to_csv("paper/data/speed_LD_microjax.csv", index=False)
print(df)
