...
def image_area0(w_center, rho, z_init, dy, carry, **_param):
    """
    inverse-ray for x-direction using lax.while_loop
    """
    q, s = _param["q"], _param["s"]
    ...
    carry_init = (yi, z_current, dx, count_x, count_all, xmax, xmin, area_x, y, dys, indx, Nindx, x0, w_center, rho2, CM2MD, a, e1, incr, incr_inv, max_iter, dz2)
    result = jax.lax.while_loop(cond_fn, update_fn, carry_init)
    
    count_all, carry = result[4], result[:]
    return count_all, carry

    
def _update_inside_source(args):
    yi, z_current, dx, count_x, xmax, dz, incr = args
    ...
    xmax = jax.lax.cond((dx == -incr) & (count_x == 0.0),
                        ...
                        lambda _: xmax)
    return yi, z_current, dx, count_x, xmax

def _update_outside_source(args):
    (yi, z_current, dx, count_x, count_all, xmax, xmin, area_x, y, dys, 
     indx, Nindx, x0, incr, dz2_last, rho2, incr_inv, max_iter, dy) = args
    ...
    def negative_run_fn(args):
        yi, z_current, dx, count_x, count_all, xmax, xmin, area_x, y, dys, indx, Nindx, incr, x0, dy = args
        ...
        def not_counted_fn(args):
            ...
            for j in range(Nindx[y_index]):
                ind = indx[y_index][j]
                if xmin[yi] < xmax[ind] and xmax[yi] > xmin[ind]:
                    return already_counted_fn((yi, indx, Nindx, xmax, xmin, area_x, y, dys))
        ...
        yi, indx, Nindx, xmax, xmin, area_x, y, dys, dx, x0, z_current, count_x = jax.lax.cond(inner_cond_fn(None),
            already_counted_fn,
            not_counted_fn,
            (yi, indx, Nindx, xmax, xmin, area_x, y, dys))
    ...
    return jax.lax.cond(dx == incr,
                        positive_run_fn,
                        negative_run_fn,
                        (yi, z_current, dx, count_x, count_all, xmax, xmin, area_x, y, dys, indx, Nindx, incr, x0, dy))

def update_fn(carry):
    yi, z_current, dx, count_x, count_all, xmax, xmin, area_x, y, dys, indx, Nindx, x0, w_center, rho2, CM2MD, a, e1, incr, incr_inv, max_iter, dz2_last = carry
...
def cond_fn(carry):
    yi, z_current, dx, count_x, count_all, xmax, xmin, area_x, y, dys, indx, Nindx, x0, w_center, rho2, CM2MD, a, e1, incr, incr_inv, max_iter, dz2 = carry
