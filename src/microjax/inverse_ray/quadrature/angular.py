...
from .rules import G15_W_ON_GK31, GK31_W, GK31_X
...
def _integrate_interval(integrand: Callable[[Array], Array], lower: Array, upper: Array) -> tuple[Array, Array, Array]:
    ...
    gauss = jnp.sum(jnp.asarray(G15_W_ON_GK31, dtype=dtype) * jacobian * values)
...
def _integrate_interval_subdivided(
    ...
    value, error, finite = jax.vmap(lambda lo, hi: _integrate_interval(integrand, lo, hi))(edges[:-1], edges[1:])
    ...
    *,
    ...
    def integrate_nonempty(bounds):
        ...
        values, errors, finite = jax.vmap(
            lambda pair: _integrate_interval_subdivided(integrand, pair[0], pair[1], subdivisions)
        )(safe_bounds)
        value = jnp.sum(jnp.where(active, values, 0.0))
        embedded_error = jnp.sum(jnp.where(active, errors, 0.0))
        error = embedded_error + jnp.abs(endpoint_value_bound) * intervals.error
        all_finite = jnp.all(jnp.where(active, finite, True)) & jnp.isfinite(error)
