...
from ..geometry.topology import RADIAL_OK, RADIAL_TOLERANCE

Array = jnp.ndarray
# Each entry is the total child count over the original topology cell. The
# short static schedule preserves JIT shapes while refining only failed cells.
...
def _sine_squared_nodes(lower: Array, upper: Array, nodes: Array) -> tuple[Array, Array]:
    """Map canonical Gauss nodes to a cell and return radii and Jacobians."""

    dtype = jnp.asarray(lower).dtype
    angle = 0.25 * jnp.pi * (jnp.asarray(nodes, dtype=dtype) + 1.0)
    width = upper - lower
    radii = lower + width * jnp.sin(angle) ** 2
    jacobian = 0.25 * jnp.pi * width * jnp.sin(2.0 * angle)
    return radii, jacobian


def _evaluate_nodes(integrand, radii: Array, interval_parameter: Array | None):
    """Vectorize an integrand over mapped nodes with an optional chart parameter."""

    if interval_parameter is None:
        return jax.vmap(integrand)(radii)
    return jax.vmap(lambda radius: integrand(radius, interval_parameter))(radii)
...
def _quadrature_rule(
    ...
    dtype = jnp.asarray(lower).dtype
    radii, jacobian = _sine_squared_nodes(lower, upper, nodes)
    evaluated = _evaluate_nodes(integrand, radii, interval_parameter)
    combined_weight = jnp.asarray(weights, dtype=dtype) * jacobian
...
def _integrate_cell(
    ...
    radii, jacobian = _sine_squared_nodes(lower, upper, GK31_X)
    evaluated = _evaluate_nodes(integrand, radii, interval_parameter)
    ...
    propagated_error = jnp.sum(jnp.abs(kronrod_combined_weight) * evaluated.error)
...
def _refine_cell_uniform(
    ...
    if interval_parameter is None:
        children = jax.vmap(lambda lo, hi: _integrate_cell(integrand, lo, hi))(edges[:-1], edges[1:])
    else:
        children = jax.vmap(lambda lo, hi: _integrate_cell(integrand, lo, hi, interval_parameter))(
            edges[:-1], edges[1:]
        )
...
def _chunked_refined_cells(
    ...
    interval_chunks = jnp.pad(intervals, ((0, pad), (0, 0))).reshape(-1, chunk_size, 2)
    ...
    if interval_parameters is not None:
        parameter_chunks = jnp.pad(interval_parameters, (0, pad)).reshape(
            (-1, chunk_size) + interval_parameters.shape[1:]
        )
    ...
    def evaluate(inputs):
        if interval_parameters is None:
            ...
            start, bounds, parameters = inputs

        def active(cell_bounds):
            slot_active = start + offsets < n_active
            safe_bounds = jnp.where(slot_active[:, None], cell_bounds, cell_bounds[0])
            if parameters is None:
                ...
                safe_parameters = jnp.where(
                    slot_active.reshape((chunk_size,) + (1,) * (parameters.ndim - 1)),
                    parameters,
                    parameters[0],
                )
                evaluated = jax.vmap(
                    lambda interval, parameter: _refine_cell_uniform(
                        integrand,
                        interval[0],
                        interval[1],
                        subdivisions,
        ...
        return jax.lax.cond(start < n_active, active, inactive, bounds)

    map_inputs = (
        (starts, interval_chunks) if interval_parameters is None else (starts, interval_chunks, parameter_chunks)
    )
    result = jax.lax.map(evaluate, map_inputs)
    return RadialIntegral(
...
def fixed_radial_integral(
    ...
    *,
    ...
    propagated_error = jnp.sum(jnp.where(active, cells.propagated_error, 0.0))
    status = jnp.bitwise_or(
...
def _chunked_initial_cells(
    ...
    def evaluate(inputs):
        ...
        def active(values):
            ...
            if parameters is None:
                ...
                evaluated = jax.vmap(lambda bounds, parameter: integrate_cell(integrand, *bounds, parameter))(
    ...
    map_inputs = (starts, chunks) if interval_parameters is None else (starts, chunks, parameter_chunks)
    result = jax.lax.map(evaluate, map_inputs)
    return RadialIntegral(
        result.value.reshape(-1)[:capacity],
        result.error.reshape(-1)[:capacity],
...
def adaptive_radial_integral(
    ...
    *,
    ...
    if max_subdivisions not in (1, 2, 4, 8, 16):
        raise ValueError("max_subdivisions must be one of 1, 2, 4, 8, or 16")

    initial = _chunked_initial_cells(integrand, intervals, n_intervals, chunk_size)
    active = jnp.arange(intervals.shape[0]) < n_intervals
    ...
    def replace_failed(current, needs_refinement, subdivisions):
        ...
        propagated = propagated.at[refine_indices].set(compact_refined.propagated_error)
        status = status.at[refine_indices].set(compact_refined.status)
        return RadialIntegral(value[:-1], error[:-1], propagated[:-1], status[:-1])
    ...
    def cells_needing_refinement(current):
        ...
        tolerance = absolute_tolerance + relative_tolerance * jnp.abs(value)
        global_failure = (status == RADIAL_OK) & jnp.isfinite(error) & (error > tolerance)
        local_tolerance = tolerance * widths / total_width
        return global_failure & active & (current.status == RADIAL_OK) & (current.error > local_tolerance)
    ...
    propagated_error = jnp.sum(jnp.where(active, refined.propagated_error, 0.0))
    status = jnp.bitwise_or(
        jnp.asarray(initial_status, dtype=jnp.int32),
    ...
    status = jnp.bitwise_or(
        status,
        jnp.where(
            jnp.isfinite(error) & (error <= absolute_tolerance + relative_tolerance * jnp.abs(value)),
            jnp.int32(RADIAL_OK),
            jnp.int32(RADIAL_TOLERANCE),
        ),
    )
