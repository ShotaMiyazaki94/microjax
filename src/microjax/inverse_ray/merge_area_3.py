...
def _refine_addmargin_merge_r(r_limb, r_ranges, margin=0.0):
    ...
    r_ranges_new = jnp.array([r_mins_new, r_maxs_new]).T
    return r_ranges_new
...
def _merge_final(r_map, th_map):
    ...
    #th_min_new = jnp.where((th_maxs == 2 * jnp.pi) & (th_min_new != 0), 0.0, th_min_new)
    
    mask_zero_fill = (th_maxs == 2 * jnp.pi) & (th_min_new != 0) & jnp.any((same_r)&(r_min_col == 0), axis=1) 
    new_matrix = jnp.stack([r_mins, r_maxs, th_min_new, th_maxs], axis=1)  # (N, 4)
    new_matrix = jnp.where(mask_zero_fill[:, None], 0.0, new_matrix)  # (N, 4) problematic!!!
