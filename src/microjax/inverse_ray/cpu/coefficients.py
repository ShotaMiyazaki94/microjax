"""Numerically stable binary-lens polynomial coefficients for CPU ICRS.
...
class BinaryQuintic(NamedTuple):
    ...
    ``coefficients`` are ordered from degree five to degree zero and solve for
    ``z_planet``.  The public image coordinate is
    ``z_binary_com = z_planet + image_shift``.
    """
...
def binary_quintic_coefficients(
    ...
    *,
    ...
    ----------
    w
        Source coordinate in the public binary centre-of-mass frame.
    s
        Positive binary separation.
    q
        Secondary-to-primary mass ratio.  The intended convention is
        ``0 < q <= 1``.

    Returns
    -------
    BinaryQuintic
        Descending polynomial coefficients and the planet-centred-to-COM
        image shift.

    Notes
    -----
    This function performs no Python-side range validation so that ``s`` and
    ``q`` may be JAX tracers.  Public callers are responsible for enforcing
    the documented lens convention.
    """
    ...
    # Work in the planet-centred image frame used by the factorized
    # polynomial.  The public source coordinate remains in the binary COM
    # frame, so the same positive shift converts solved images back to COM.
    ...
    # This real/imaginary factorization is the key small-q stabilization: the
    # coefficient stays assembled from terms at its physical scale near a
    # planetary caustic.
    c2 = (
        factored_scale.real
        * (
