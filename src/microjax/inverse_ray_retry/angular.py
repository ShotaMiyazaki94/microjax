...
from .levelset import binary_level_set_fourier, triple_level_set_fourier
...
def evaluate_fourier(coefficients: Array, theta: Array) -> Array:
    ...
    return jnp.real(coefficients[0]) + 2.0 * jnp.sum(
        jnp.real(oscillatory), axis=-1
    )
...
def evaluate_fourier_derivative(coefficients: Array, theta: Array) -> Array:
    ...
    oscillatory = coefficients[1:] * jnp.exp(1j * theta[..., None] * modes)
    return 2.0 * jnp.sum(
        jnp.real(1j * modes * oscillatory), axis=-1
    )


def evaluate_fourier_second_derivative(
    coefficients: Array, theta: Array
) -> Array:
    ...
    oscillatory = coefficients[1:] * jnp.exp(1j * theta[..., None] * modes)
    return -2.0 * jnp.sum(
        jnp.real(modes**2 * oscillatory), axis=-1
    )
...
def _conditioned_root_angle_error(
    ...
    """Estimate angular root uncertainty in simple and tangent regimes.

    The usual first-order condition estimate ``delta_H / abs(H')`` diverges
    when two physical boundary roots coalesce at a radial tangency.  In the
    curvature-dominated regime, the local second-order model

    ``delta_H = abs(H') * delta + 0.5 * abs(H'') * delta**2``

    has a stable positive solution and recovers the expected
    ``sqrt(delta_H)`` scaling.  Well-conditioned simple roots retain the
    historical linear estimate.  This remains a local numerical estimator,
    not an interval-arithmetic proof; vanishing curvature therefore falls back
    to the conservative linear path.
    """
    ...
    discriminant = jnp.sqrt(
        slope**2 + 2.0 * curvature * uncertainty
    )
    quadratic = 2.0 * uncertainty / jnp.maximum(
        slope + discriminant, jnp.finfo(uncertainty.dtype).tiny
    )
    curvature_resolved = curvature > jnp.sqrt(eps)
    return jnp.where(
        curvature_resolved, jnp.minimum(quadratic, linear), linear
    )
...
def _angular_intervals_fourier_roots(
    ...
    def constant_result(_):
        ...
        intervals = jnp.zeros(
            (interval_capacity, 2), dtype=jnp.asarray(r).dtype
        )
...
def _angular_intervals_fourier_roots_impl(
    ...
    polynomial = jnp.concatenate(
        (c[:0:-1], c[:1], jnp.conjugate(c[1:]))
    )
    root_solver = (
        poly_roots_self_inversive_robust_fixed
        if robust_roots
        else poly_roots_self_inversive_fixed
    )
    ...
    residual_tolerance = (
        fourier.padding
        + root_refinement_roundoff
        + fourier_evaluation_roundoff
    )
    ...
    check_angles = (
        2.0
        * jnp.pi
        * jnp.arange(16, dtype=jnp.asarray(r).dtype)
        / 16.0
    )
    ...
    in_interval = (
        valid
        & (theta_in_interval >= theta_min)
        & (theta_in_interval <= theta_max)
    )
    roots_sorted = jnp.sort(
        jnp.where(in_interval, theta_in_interval, theta_max)
    )
    boundaries = jnp.concatenate(
        (jnp.atleast_1d(theta_min), roots_sorted, jnp.atleast_1d(theta_max))
    )
    ...
    previous_active = jnp.concatenate(
        (jnp.asarray([False]), segment_active[:-1])
    )
    next_active = jnp.concatenate(
        (segment_active[1:], jnp.asarray([False]))
    )
    ...
    intervals = jnp.stack(
        (segment_lo[start_indices], segment_hi[end_indices]), axis=1
    )
    n_intervals = jnp.minimum(
        n_intervals_raw, jnp.int32(interval_capacity)
    )
    intervals = jnp.where(
    ...
    if propagate_coefficient_padding:
        root_error = (residual + fourier.padding) / jnp.maximum(
            jnp.abs(derivative), jnp.sqrt(eps)
        )
    else:
    ...
    root_order = jnp.argsort(
        jnp.where(in_interval, theta_in_interval, theta_max)
    )
    sorted_root_error = jnp.where(
        in_interval[root_order], root_error[root_order], 0.0
    )
    ...
    # measure uncertainty.
    measure_error = jnp.sum(
        jnp.where(interval_start, boundary_error[:-1], 0.0)
    ) + jnp.sum(jnp.where(interval_end, boundary_error[1:], 0.0))
    error = jnp.minimum(
        measure_error,
...
def angular_measure_binary_roots(
    ...
    *,
    ...
    active = (
        jnp.arange(_BINARY_INSIDE_INTERVAL_CAPACITY) < result.n_intervals
    )
