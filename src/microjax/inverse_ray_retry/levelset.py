"""Denominator-cleared source-boundary level sets for inverse ray shooting.
...
def _normalized_fourier_level_set(
    samples: Array, degree: int
) -> FourierLevelSet:
    ...
    raw_scale = jnp.abs(coefficients[0]) + 2.0 * jnp.sum(
        jnp.abs(coefficients[1:])
    )
...
def binary_level_set(
    ...
    *,
    ...
    numerator = (
        (z - w_center_shifted) * denominator
        - e1 * d_minus
        - (1.0 - e1) * d_plus
    )
    return jnp.real(
        numerator * jnp.conjugate(numerator)
        - rho**2 * denominator * jnp.conjugate(denominator)
    )
...
def binary_level_set_fourier(
    ...
    *,
    ...
    source_offset = midpoint_offset - jnp.asarray(
    ...
    numerator_modes = jnp.stack(
        [positive_mode(numerator, mode) for mode in range(4)]
    )
    denominator_modes = jnp.stack(
        [
            positive_mode(denominator, mode) if mode < 3 else 0.0j
            for mode in range(4)
        ]
    )
...
def triple_level_set_fourier(
    ...
    *,
    ...
    angles = (
        2.0
        * jnp.pi
        * jnp.arange(TRIPLE_FOURIER_SAMPLES, dtype=real_dtype)
        / TRIPLE_FOURIER_SAMPLES
    )
