...
def _linear_ld_cumulative_profile(r: jnp.ndarray, a: jnp.ndarray) -> jnp.ndarray:
    mu = jnp.sqrt(1 - r**2)
    return (a * (mu**2 - 2 / 3 * mu**3) - mu**2) / (1 - a / 3) + 1
...
def binary_mag(
    ...
    **Returns**
    ...
    use_limb_darkening = limb_darkening_coeff is not None
    ld_coeff = 0.0 if limb_darkening_coeff is None else limb_darkening_coeff

    mag = _extended_light_curve_impl(
        trajectory_l,
        s,
        q,
        rho,
...
def _extended_light_curve_impl(
    trajectory_l,
    s,
    q,
    rho,
    ...
    analytic: bool = True,
    return_info: bool = False,
    limb_darkening_coeff: float = 0.0,
    use_limb_darkening: bool = False,
    n_annuli: int = 10,
):
    use_limb_darkening = jnp.asarray(use_limb_darkening)
    limb_darkening_coeff = jnp.asarray(limb_darkening_coeff)
    ...
    def contour_uniform(traj):
        return contour_integral(
            traj, tol, retol, rho, s, q, default_strategy, analytic
        )

    def contour_limb_darkening(traj):
        cumulative_profile = _linear_ld_cumulative_profile(
            rho_frac, limb_darkening_coeff
        )
        surface_brightness = jnp.diff(cumulative_profile, prepend=0) / jnp.diff(
            rho_frac2, prepend=0
        )

        mag_contour_rho_fun = lambda rho_val: contour_integral(
            traj, tol, retol, rho_val, s, q, default_strategy, analytic
        )
    ...
    mag_contour = lambda traj: lax.cond(
        use_limb_darkening, contour_limb_darkening, contour_uniform, traj
    )
...
def extended_light_curve(
    ...
    **Parameters**
    ...
    - `limb_darkening_coeff`: Linear limb-darkening coefficient. Set to None for uniform brightness.
    ...
    return _extended_light_curve_impl(
        trajectory_l,
        s,
        q,
        rho,
