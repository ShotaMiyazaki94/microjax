from microjax.fastlens.special import ellipk, ellipe, gamma, digamma_
from microjax.fastlens.special import j0, j1, j2, j1p5
from scipy.special import ellipe as ellipe_s, ellipk as ellipk_s, gamma as gamma_s, digamma as digamma_s
...
plt.figure()
plt.plot(jnp.real(random_complex),jnp.abs(digamma_jax - digamma_scipy),".")
...
plt.show()
plt.figure()
plt.plot(jnp.imag(random_complex),jnp.abs(digamma_jax - digamma_scipy),".")
...
print("digamma results are close :", jnp.allclose(digamma_jax, digamma_scipy, atol=atol))
...
time_digamma_jax = timeit.timeit(lambda: digamma(random_m_values_jax), number=n_runs)
time_digamma_scipy = timeit.timeit(lambda: digamma_s(random_m_values), number=n_runs)
...
print(f"gamma SciPy time   : {time_digamma_scipy / n_runs:.6f} s")
print(f"digamma JAX time   : {time_digamma_jax / n_runs:.6f} s")
print(f"digamma SciPy time : {time_gamma_scipy / n_runs:.6f} s")
