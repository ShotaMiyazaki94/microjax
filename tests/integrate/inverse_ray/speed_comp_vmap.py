from jax import config 
config.update('jax_enable_x64', True) 
import pandas as pd
import numpy as np
import time
import jax.numpy as jnp
from jax import grad, jit, vmap, random
from microjax.inverse_ray.lightcurve import mag_lc, mag_lc_vmap
from microjax.trajectory import dtn_dum_parallax, _get_info_parallax

# initial guess
_t_0, _u_0, _t_E = 6.83640951e+03, 2.24211333e-01, 1.33559958e+02 
_s, _q, _alpha = 9.16157288e-01, 5.87559438e-04, 1.00066409e+02
_rho, _pi_E_N, _pi_E_E = 2.44003713e-03, 1.82341182e-01,9.58542572e-02
_fs, _fb = 8.06074085e-01, 8.62216897e-01
_f_sum = _fs + _fb

from astropy.coordinates import SkyCoord
coords = SkyCoord("18:02:29.21 −28:23:46.5", unit=("hourangle", "deg"))
coords_deg = (coords.ra.deg, coords.dec.deg)
tref = 6836.0
info_parallax = _get_info_parallax(RA=coords_deg[0], Dec=coords_deg[1], tref=tref)
t_peri, qne0, vne0, xpos, ypos, north, east = info_parallax

N_points = 5000

t = jnp.linspace(6300, 7000, N_points)
_alpha_rad = jnp.deg2rad(_alpha)
tau = (t - _t_0)/_t_E 
um  = _u_0 
#dtn, dum = dtn_dum_parallax(t, _pi_E_N, _pi_E_E, t_peri, qne0, vne0, xpos, ypos, north, east)
#tau = (t - _t_0)/_t_E + dtn
#um  = _u_0 + dum 
y1 = -um*jnp.sin(_alpha_rad) + tau*jnp.cos(_alpha_rad)
y2 = um*jnp.cos(_alpha_rad) + tau*jnp.sin(_alpha_rad)
w_points = jnp.array(y1 + y2 * 1j, dtype=complex)

r_resolution = 500
th_resolution = 500

mag_lc(w_points, _rho, s=_s, q=_q, cubic=True, r_resolution=r_resolution, th_resolution=th_resolution).block_until_ready()
start_time = time.time()
lc = mag_lc(w_points, _rho, s=_s, q=_q, cubic=True, r_resolution=r_resolution, th_resolution=th_resolution).block_until_ready()
end_time = time.time()
print(f"Execution time for mag_lc: {1000 * (end_time - start_time)/N_points:.3f} ms per datapoint")

mag_lc_vmap(w_points, _rho, s=_s, q=_q, cubic=True, r_resolution=r_resolution, th_resolution=th_resolution)
start_time = time.time()
lc_vmap = mag_lc_vmap(w_points, _rho, batch_size=8000, s=_s, q=_q, cubic=True, r_resolution=r_resolution, th_resolution=th_resolution).block_until_ready()
end_time = time.time()
print(f"Execution time for mag_lc_vmap: {1000 * (end_time - start_time)/N_points:.3f} ms per datapoint")