...
def cluster_1d(arr, bins=100, max_cluster=5):
    bin_min, bin_max = arr.min(), arr.max()
    delta = (bin_max - bin_min) / bins
    bin_edges = jnp.linspace(bin_min - delta, bin_max + delta, bins, endpoint=True)
    bin_indices = jnp.digitize(arr, bin_edges) - 1
    bin_indices = jnp.clip(bin_indices, 0, bins - 1)
    counts = jnp.bincount(bin_indices, length=bins)
    bin_mask = counts > 0 

    diff_mask = jnp.diff(bin_mask.astype(int))[:-1]  #length: bin + 1
    start_mask = diff_mask == -1 
    end_mask   = diff_mask == 1

    start_edges = jnp.sort(bin_edges[1:-1] * start_mask.astype(float), descending=True)[:max_cluster]
    end_edges   = jnp.sort(bin_edges[1:-1] * end_mask.astype(float), descending=True)[:max_cluster] 
...
if(1):
    ...
    r     = jnp.ravel(jnp.abs(image_limb))
    theta = jnp.ravel(jnp.mod(jnp.arctan2(image_limb.imag, image_limb.real), 2*jnp.pi))
    r_mins, r_maxs = cluster_1d(r)
