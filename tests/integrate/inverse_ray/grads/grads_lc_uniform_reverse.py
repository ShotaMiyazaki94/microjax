...
def get_mag(params):
    ...
    mag_func = lambda w: mag_binary(w, rho, s=s, q=q, 
...
mag_jac = jit(jacrev(lambda params: get_mag(params)[1]))
...
fig.savefig(f"tests/integrate/inverse_ray/grads/grads_lc_uniform_r{r_resolution}_{th_resolution}_rev.pdf", bbox_inches="tight")
print(f"tests/integrate/inverse_ray/grads/grads_lc_uniform_r{r_resolution}_{th_resolution}_rev.pdf")
plt.close()