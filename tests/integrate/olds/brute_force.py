...
print("time (source_limb_image):", end - start)
...
#r_ = jnp.array([jnp.linspace(x1, x2, 100)*jnp.float_(mask) for (x1, x2), mask in zip(r_,r_mask)]) 
r_ = jnp.concatenate([jnp.arange(x1, x2, dr) for x1, x2 in r_[r_mask]])

# Theta range construction
...
print("time (overlap etc...):", end - start)
print(r_.shape, th_.shape, r_.shape[0] * th_.shape[0])

# Third block: Grid construction
start = time.time()
