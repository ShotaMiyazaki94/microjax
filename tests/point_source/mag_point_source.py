...
tE = 100 # einstein radius crossing time
...
mag = mag_point_source(w, nlenses=3, q=q, s=s, q3=q3, r3=jnp.abs(r3), psi=psi)
crit, cau = critical_and_caustic_curves(npts=1000, nlenses=3, q=q, s=s, q3=q3, r3=jnp.abs(r3), psi=psi)
...
ax[1].scatter(cau.ravel().real, cau.ravel().imag,   marker=".", color="r", s=1)
ax[1].scatter(crit.ravel().real, crit.ravel().imag, marker=".", color="g", s=1)
ax[1].axis("equal")
plt.show()