...
def test_triple_denominator_cleared_level_set_matches_lens_equation():
    ...
    rho = 0.03
    z_cm = jnp.asarray(
        [0.2 + 0.3j, -0.7 + 0.1j, 0.4 - 0.5j, 1.2 + 0.6j]
    )
    ...
    denominator = (
        (zbar - geometry.a)
        * (zbar + geometry.a)
        * (zbar - jnp.conjugate(geometry.r3_complex))
    )
    expected = rational * jnp.abs(denominator) ** 2
    assert np.allclose(
        np.asarray(cleared), np.asarray(expected), rtol=2e-13, atol=2e-15
    )
...
def test_triple_degree_four_fourier_level_set_matches_direct_evaluation():
    ...
    raw_scale = jnp.abs(fourier.coefficients[0]) + 2.0 * jnp.sum(
        jnp.abs(fourier.coefficients[1:])
    )
    ...
    reconstructed_reference = evaluate_fourier(
        fourier.coefficients, reference_angle
    )
...
def test_triple_degree_eight_roots_match_dense_angular_oracle():
    ...
    assert np.isclose(
        float(result.measure), float(dense_measure), rtol=0.0, atol=4e-5
    )
...
def test_triple_degree_eight_root_gradient_matches_forward_and_reverse():
    ...
    assert np.isclose(
        float(forward), float(finite_difference), rtol=1e-8, atol=1e-10
    )
