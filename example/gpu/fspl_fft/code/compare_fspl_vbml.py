"""Compare JAX FSPL magnification against VBMicrolensing.

Outputs fspl_vs_vbml.png showing, for a Paczynski trajectory u(t):
 - Top row: magnification curves A(t) (per rho, and per LD coefficient)
 - Bottom row: relative residuals |A_FSPL - A_VBML|/A_VBML (log scale)
 for uniform disk (left) and linear limb-darkening (right).
"""

# JAX configuration and the local source path must be set before related imports.
# ruff: noqa: E402
...
try:
    ...
    print("VBMicrolensing not installed; run `pip install VBMicrolensing`.")
    sys.exit(0)
...
def eval_curve(mag_fn, vbml_fn, rhos: Sequence[float], t: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray, float]:
    ...
    for i, rho in enumerate(rhos):
        def vb_fn(u):
            return vbml_fn(float(u), float(rho))
...
def main():
    ...
    table_path = VBMicrolensing.__file__.replace("__init__.py", "data/ESPL.tbl")
    ...
    ax_res_d.set_ylabel("rel. error vs VBML")
    ...
    fig.tight_layout()
    out = Path(__file__).resolve().parent.parent / "outputs" / "fspl_vs_vbml.png"
    out.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(out, dpi=150)
    print(f"saved {out}")
    print("CPU timings (per rho, ms):")
    for rho, tref, tfspl in per_rho_disk:
        slowdown = (tfspl / max(tref, 1e-9))
        print(f"  disk rho={rho:.3g}: VBML {tref*1e3:.2f}ms, FSPL {tfspl*1e3:.2f}ms, x{slowdown:.1f}")
    for a1, rho, tref, tfspl in per_rho_ld_all:
        slowdown = (tfspl / max(tref, 1e-9))
        print(f"  LD1 a1={a1:.1f} rho={rho:.3g}: VBML {tref*1e3:.2f}ms, FSPL {tfspl*1e3:.2f}ms, x{slowdown:.1f}")
