"""Compare uniform and limb-darkened GPU ICRS on one binary-lens track."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare uniform and limb-darkened GPU ICRS in one figure."
    )
    ...
    parser.add_argument("--n-limb", type=int, default=BinaryMagConfig().n_limb)
...
def evaluate_profile(
    ...
    # Compile before measuring steady-state GPU execution.
    evaluate_gpu().block_until_ready()
    ...
    gpu_result, gpu_seconds = median_runtime(evaluate_gpu, repeats=args.repeats)
    gpu = np.asarray(gpu_result)
    certified = np.isfinite(gpu)
    if not np.any(certified):
        raise RuntimeError(f"{name}: GPU ICRS returned no finite magnifications")
    relative_error = np.abs(gpu - vbml) / vbml
...
def profile_summary(
    ...
    *,
    ...
    summary = {
        ...
        "microjax_ms_per_point": 1.0e3 * result.gpu_seconds / args.points,
        ...
        "gpu_over_vbml": result.gpu_seconds / result.vbml_seconds,
        "nonfinite": int(np.sum(~result.certified)),
        "finite_residuals_above_rtol": residuals_above_target,
        ...
        "finite_relative_error": residual_statistics,
        "finite_vbml_reference_relative_error": reference_residual_statistics,
    }
    print(f"\n{result.name}")
    print(
    ...
    print(
        "computation time: %.3f sec (%.3f ms per point), median of %d, "
        "with microJAX one-shot GPU ICRS"
        % (result.gpu_seconds, summary["microjax_ms_per_point"], args.repeats)
    )
    print(
        "GPU / VBML steady-state ratio (Tol=RelTol=%.1e): %.2fx"
        % (VBML_TIMING_TOL, summary["gpu_over_vbml"])
    )
    print("  non-finite GPU results:", summary["nonfinite"])
    print(
        "  finite-result relative error (VBML Tol=RelTol=%.1e): "
        "median=%.3e, p95=%.3e, p99=%.3e, max=%.3e"
        % (
            VBML_RESIDUAL_TOL,
            *(
                summary["finite_relative_error"][key]
                for key in ("median", "p95", "p99", "maximum")
            ),
        )
    )
    print(
        "  finite residuals above rtol:",
        residuals_above_target,
    )
    print(
        ...
        % (
            ...
            VBML_RESIDUAL_TOL,
            *(
                summary["finite_vbml_reference_relative_error"][key]
                for key in ("median", "p95", "p99", "maximum")
            ),
        )
    )
...
def write_maximum_residual_diagnostic(
    ...
    candidates = np.flatnonzero(result.certified)
...
def main() -> None:
    ...
    if residuals_above_target:
        raise RuntimeError(
            ...
            f"{residuals_above_target} finite residuals above rtol"
        )
