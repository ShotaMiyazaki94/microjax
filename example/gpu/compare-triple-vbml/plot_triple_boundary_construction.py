"""Plot boundary-root ICRS geometry for one triple-lens source position."""
...
from microjax.inverse_ray.roots.angular import angular_intervals_triple_roots
...
def plot_triple_boundary_construction(
    w_center: complex,
    rho: float,
    *,
    s: float,
    ...
    w_shifted = w_center - geometry.shifted
    ...
    lens_positions_midpoint = jnp.asarray(
        [geometry.a, -geometry.a, geometry.r3_complex]
    )
    ...
    angular = jax.jit(
        jax.vmap(
            lambda radius: angular_intervals_triple_roots(
                radius,
                0.0,
                2.0 * jnp.pi,
                w_shifted,
    ...
    lens_positions = np.asarray(
        [
            -geometry.a + geometry.shifted,
            geometry.a + geometry.shifted,
            geometry.r3_complex + geometry.shifted,
        ]
    )
    image_axis.scatter(
        lens_positions.real,
    ...
    fig.suptitle(
        ...
        rf"{profile_label}; $s={s:g}$, $q={q:g}$, $q_3={q3:g}$, $\rho={rho:g}$"
    )
...
__all__ = ["plot_triple_boundary_construction"]
