...
CPU counterpart of `example/gpu/compare-binary-vbml`. One script evaluates
...
```bash
python example/cpu/compare-binary-vbml/code/compare_binary_profiles.py
```
...
VBMicrolensing 5.5 stores the linear coefficient in `solver.a1`;
`BinaryMag2` then applies that profile when it dispatches to the finite-source
solver. Its `BinaryMagDark` Python docstring labels the sixth argument as
`a1`, but the bundled C++ implementation defines that argument as the
calculation tolerance, so the comparison deliberately uses the stateful
coefficient API.
